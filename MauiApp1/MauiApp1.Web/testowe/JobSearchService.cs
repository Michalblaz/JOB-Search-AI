    ...
    public class JobSearchService
    {
        ...
        private async Task<List<JobOffer>> FetchAdzunaOffersAsync(string query, string location, bool remoteOnly)
        {
            ...
            var queryVariants = remoteOnly
                ...
                : BuildSearchQueries(effectiveQuery);
        ...
        private async Task<List<JobOffer>> FetchRemotiveOffersAsync(string query, bool remoteOnly)
        {
            try
            {
                var queryVariants = string.IsNullOrWhiteSpace(query)
                    ? new List<string> { string.Empty }
                    : BuildSearchQueries(query);
                var collectedOffers = new List<JobOffer>();

                foreach (var queryVariant in queryVariants)
                {
                    var remotiveUrl = string.IsNullOrWhiteSpace(queryVariant)
                        ? "https://remotive.com/api/remote-jobs"
                        : $"https://remotive.com/api/remote-jobs?search={Uri.EscapeDataString(queryVariant)}";
                    ...
                    collectedOffers.AddRange(data.Jobs
        ...
        private async Task<List<JobOffer>> FetchArbeitnowOffersAsync(string query, bool remoteOnly)
        {
            try
            {
                ...
                return data.Data
                    ...
                    .Where(offer => !remoteOnly || offer.IsRemote)
                    .Where(offer => MatchesSearchQuery(offer, normalizedQuery))
                    .GroupBy(GetOfferKey)
                    .Select(group => group.First())
                    .ToList();
            }
